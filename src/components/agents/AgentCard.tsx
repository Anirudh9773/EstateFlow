...
export default function AgentCard({ agent }: AgentCardProps) {
  return (
    <Card className="border-ef-border bg-card shadow-none p-6 hover:border-gold/30 transition-colors duration-150">
      <div className="flex items-start gap-4 mb-4">
        <Avatar className="h-14 w-14">
          <AvatarImage src={agent.avatar} alt={`${agent.name} profile photo`} />
          <AvatarFallback className="bg-navy text-gold font-medium text-sm">
            {getInitials(agent.name)}
          </AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-semibold text-navy truncate">{agent.name}</h3>
            {agent.verified && (
          ...
          <p className="text-text-secondary text-sm truncate">{agent.agency}</p>
        </div>
      </div>

      <div className="flex items-center gap-2 mb-3">
        <StarRating rating={agent.rating} size="sm" />
        <span className="text-navy font-medium text-sm">{agent.rating}</span>
      ...
      <Link href={`${ROUTES.agents}/${agent.id}`}>
        <Button variant="outline" className="w-full border-navy text-navy hover:bg-navy hover:text-gold">
          View profile
        </Button>
      </Link>
    </Card>
